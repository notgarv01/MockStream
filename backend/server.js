...
// List endpoints
fastify.get('/v1/endpoints', async (request, reply) => {
...
fastify.post('/v1/endpoints', async (request, reply) => {
  ...
  const newEndpoint = await db.createEndpoint(endpointId, token, name || 'Unnamed Endpoint');
  return newEndpoint;
});

// Get webhook history
fastify.get('/v1/endpoints/:endpoint_id/webhooks', async (request, reply) => {
  const { endpoint_id } = request.params;
